package io.papermc.yarn.decompiler
...
fun main(args: Array<String>) {
    AbstractForkedFFExecutor.decompile(args, ForgedFlowerForkedFFExecutor)
}

object ForgedFlowerForkedFFExecutor : AbstractForkedFFExecutor() {
    override fun runFF(
        ...
        options[IFernflowerPreferences.LOG_LEVEL] = "ERROR"

        val saver = ThreadSafeResultSaver({ output }, { lineMap })
        val logger = ThreadIDFFLogger()
        val ff = Fernflower(
            { ext, int -> FernFlowerUtils.getBytecode(ext, int) },
            ForgedFlowerThreadSafeResultSaver(saver),
            options,
...
private typealias ForgedSeverity = IFernflowerLogger.Severity
